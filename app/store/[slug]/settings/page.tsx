...
export default async function SettingsPage({ params }: Props) {
    ...
    // On custom domains, auth was already verified by middleware
    // On subdomain/platform, verify normally
    const middlewareAuth = headersList.get('x-middleware-auth')

    const { data: { user } } = await supabase.auth.getUser()
    if (!user && middlewareAuth !== 'verified') {
        redirect(`/auth/login?redirect=/${slug}/settings`)
    }
