...
const inputCls = cn(
    'w-full h-11 rounded-xl border border-border bg-muted/30',
        ...
        >
                ...
                <section className="space-y-4 rounded-2xl border border-border bg-background p-6">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <p className="text-[11px] text-muted-foreground">Used in the organization URL.</p>
                    </div>

                    <div className="space-y-1.5">
                        <label className="block text-[12px] font-medium text-foreground">Logo URL</label>
                        <input type="text" value={logoUrl} onChange={e => setLogoUrl(e.target.value)} placeholder="https://…" className={inputCls} />
                    </div>

                    {error && (
                        <div className="flex items-center gap-2 text-[12px] text-destructive bg-destructive/20 px-3 py-2.5 rounded-xl">
                ...
                {/* Danger zone */}
                {isOwner && (
                    <section className="space-y-3 rounded-2xl border border-[var(--md-sys-color-error)]/30 bg-destructive/20/20 p-6">
                        <h2 className="text-[11px] font-semibold uppercase tracking-widest text-destructive">Danger zone</h2>
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                className="shrink-0 h-9 px-4 rounded-full text-[12px] font-medium border border-[var(--md-sys-color-error)]/40 text-destructive hover:bg-[var(--md-sys-color-error)]/10 transition-colors"
                            >
                                Delete
                            </button>
                        </div>
